...
async function main() {
  ...
  if (listOnly) {
    const [allReps] = await conn.query(
      `SELECT id, username, display_name, tenant_id, shipping_company_id, is_active FROM users WHERE role = 'representative' ORDER BY id ASC`
    );
    console.log("كل مستخدمي representative:");
  ...
  if (process.argv.includes("--check")) {
    const checkRepId = process.env.REP_ID;
    const [rows] = await conn.query(
      `SELECT id, shipment_number, receiver_name, status, assigned_user_id, shipping_company_id, tenant_id, deleted_at, internal_notes
       FROM shipments WHERE internal_notes = ? ${checkRepId ? "AND assigned_user_id = ?" : ""} ORDER BY id DESC LIMIT 20`,
      checkRepId ? [DUMMY_TAG, checkRepId] : [DUMMY_TAG]
    );
    console.log(`عدد الشحنات الوهمية الموجودة: ${rows.length}`);
  ...
  if (undo) {
    const [manifests] = await conn.query(`SELECT id FROM shipment_manifests WHERE notes = ?`, [DUMMY_TAG]);
    for (const m of manifests) {
      await conn.execute(`DELETE FROM shipment_manifest_items WHERE manifest_id = ?`, [m.id]);
      await conn.execute(`DELETE FROM shipment_manifests WHERE id = ?`, [m.id]);
    }
    const [res] = await conn.query("DELETE FROM shipments WHERE internal_notes = ?", [DUMMY_TAG]);
    console.log(`تم حذف ${res.affectedRows} شحنة وهمية و ${manifests.length} بيان وهمي.`);
    await conn.end();
    return;
  }

  const repNameFilter = process.env.REP_NAME || "";
  const repId = process.env.REP_ID || "";
  let reps;
  if (repId) {
    [reps] = await conn.query(
      `SELECT id, username, display_name, tenant_id, shipping_company_id
       FROM users WHERE role = 'representative' AND id = ? LIMIT 1`,
      [repId]
    );
  ...
  if (!reps.length) {
    console.error("مفيش أي مستخدم مطابق. راجع الاسم أو الدور.");
    const [allReps] = await conn.query(
      `SELECT id, username, display_name, role, is_active, tenant_id, shipping_company_id FROM users WHERE role = 'representative'`
    );
    console.log("كل مستخدمي representative الموجودين:", JSON.stringify(allReps, null, 2));
    await conn.end();
    process.exit(1);
  }
  const rep = reps[0];
  ...
  let companyId = rep.shipping_company_id;
  const tenantId = rep.tenant_id;

  if (!companyId) {
    const [companies] = await conn.query(
      `SELECT id, name FROM shipping_companies WHERE tenant_id = ? OR tenant_id IS NULL ORDER BY id ASC LIMIT 1`,
      [tenantId]
    );
    if (!companies.length) {
      console.error(`المندوب ${rep.display_name} (id=${rep.id}) مش مربوط بشركة شحن، ومفيش أي شركة شحن للـ tenant ده أصلاً.`);
      await conn.end();
      process.exit(1);
    }
    companyId = companies[0].id;
    await conn.execute(`UPDATE users SET shipping_company_id = ? WHERE id = ?`, [companyId, rep.id]);
    console.log(`تم ربط المندوب ${rep.display_name} تلقائيًا بشركة الشحن: ${companies[0].name} (id=${companyId})`);
  }
  ...
  // ─── إنشاء بيان شحن (manifest) وربط كل الشحنات الوهمية بيه ────────────────
  // الـ endpoint بتاع /representative/shipments بيجيب الشحنات عن طريق البيانات
  // المرتبطة بشركة الشحن، مش عن طريق assigned_user_id مباشرة.
  const manifestNumber = "DUM-MANIFEST-" + Date.now().toString().slice(-6);
  const [manifestRes] = await conn.execute(
    `INSERT INTO shipment_manifests (tenant_id, manifest_number, shipping_company_id, status, notes, created_at)
     VALUES (?, ?, ?, 'open', ?, ?)`,
    [tenantId, manifestNumber, companyId, DUMMY_TAG, fmtDate(now)]
  );
  const manifestId = manifestRes.insertId;

  for (const shipmentId of insertedIds) {
    await conn.execute(
      `INSERT INTO shipment_manifest_items (manifest_id, shipment_id, delivery_status, added_at)
       VALUES (?, ?, 'pending', ?)`,
      [manifestId, shipmentId, fmtDate(now)]
    );
  }

  console.log(`تم إدخال ${inserted} شحنة وهمية بنجاح للمندوب: ${rep.display_name} (id=${rep.id})`);
  console.log(`تم إنشاء بيان شحن رقم ${manifestNumber} (id=${manifestId}) وربط كل الشحنات بيه.`);
