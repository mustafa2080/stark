// ══════════════════════════════════════════════════════════════════════════
// Dialog مشاركة صورة الفاتورة — يولّد صورة الفاتورة، يعرض preview، ويبعتها واتساب
// ══════════════════════════════════════════════════════════════════════════
import { useEffect, useState } from "react";
import { FileText, Download, Send, Loader2, MessageCircle, Printer } from "lucide-react";
import {
...
export function ShareInvoiceDialog({ open, onOpenChange, shipment }: Props) {
  ...
  useEffect(() => {
    ...
    setError(null);
    generateInvoiceShareImage(shipment)
      .then(setImageUrl)
      .catch((e) => {
        console.error(e);
        setError("تعذر توليد صورة الفاتورة. حاول مرة أخرى.");
      })
  ...
  const fileName = `فاتورة-${shipment.invoiceNumber ?? shipment.shipmentNumber ?? shipment.id}.png`;
    ...
    try {
      ...
      if (navigator.share && canShareFiles) {
        await navigator.share({
          files: [file],
          title: `فاتورة ${shipment.shipmentNumber ?? shipment.id}`,
        });
        return;
      }
      handleDownload();
      if (shipment.receiverPhone) {
        const link = buildWhatsAppLink(
          ...
          "فاتورة شحنتك 🧾 (الصورة تم تحميلها — أرفقها هنا)"
        );
  ...
  // ── طباعة الصورة على ورقة A4 حقيقية ──────────────────────────────────
  const handlePrint = () => {
    if (!imageUrl) return;
...
<head>
<meta charset="UTF-8" />
<title>طباعة فاتورة ${shipment.shipmentNumber ?? shipment.id}</title>
<style>
  ...
  .print-wrap { width: 100%; display: flex; justify-content: center; }
  img {
    max-width: 186mm;
    max-height: 273mm;
...
<body>
  <div class="print-wrap">
    <img src="${imageUrl}" alt="فاتورة ${shipment.shipmentNumber ?? shipment.id}" />
  </div>
  <script>
      ...
      <DialogContent className="max-w-xl" dir="rtl">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            فاتورة رسمية بشعار الشركة وكامل تفاصيل الشحن — جاهزة للمشاركة أو الطباعة
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="rounded-xl border border-border overflow-hidden bg-muted/20 flex items-center justify-center min-h-[360px] p-2">
          ...
          {imageUrl && !isGenerating && (
            <img src={imageUrl} alt="صورة الفاتورة" className="max-w-full max-h-[580px] rounded-md object-contain shadow-sm" />
          )}
        </div>
