    ...
    onSuccess: (result: any, variables: { status: "open" | "closed" }) => {
      ...
      if (manifest && variables?.status === "closed") {
        ...
        const ordersForPnlLocal = (manifest.orders ?? []).filter((o: any) => {
          ...
          // (نفس الباك اند + كارت الرصيد المستحق بالظبط).
          if (o.deliveryStatus === "partial_received") {
            if (isRolledOver(o)) return false;
            return !isStillAtShippingLocal(o);
          }
          return true;
        });
    ...
    <div className="manifest-screen print:hidden max-w-5xl mx-auto space-y-5 animate-in fade-in duration-500" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                {isLocked ? "مقفول من قبل المندوب" : "مفتوح"}
              </Badge>
            </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap print:hidden">
          ...
          {/* فتح البيان — أدمن فقط. زرار إغلاق البيان اتنقل لآخر الصفحة full-width */}
          {isLocked && isAdmin && (
            <Button
              variant="outline"
              size="sm"
      ...
      {true && (() => {
        ...
        const ordersForPnl = (manifest.orders ?? []).filter(o => {
          ...
          // مُرحَّل ([ROLLED_OVER]) — الجزء المسلَّم الفعلي اتحسب أصلًا في البيان القديم
          // على السجل الأصلي، والإشعار ده مجرد تأكيد رجوع الباقي للمخزن. فمبيضيفش أي
          // إيراد هنا حتى بعد تأكيد الاستلام (returnReceived=1) — نفس منطق الباك اند
