...
export default function ShippingManifestPage() {
  ...
  const filteredManifestOrders = useMemo(() => {
    ...
    // البيان مفتوح: المرتجع/الجزئي اللي لسه عند المندوب (غير مؤكد الاستلام)
    //   يختفي من الجدول تمامًا ويفضل في الحاوية الحمرا بس — الجدول المفتوح
    //   يقتصر على قيد الانتظار/مؤجل فقط. (بطلب صريح من المستخدم 2026-08-27.)
    // البيان مقفول: يفضل ثابت زي ما كان لحظة القفل بالظبط — كل الشحنات (بما
    //   فيها المرتجع/الجزئي) تظهر في الجدول عادي زي أي بيان قديم، من غير أي
    //   استبعاد. (تصحيح 2026-08-27: الاستبعاد كان بيتطبق غلط على المقفول
    //   كمان في تعديل سابق نفس اليوم — البيان المقفول تاريخي وميتغيرش شكله.)
    ...
    const ordersWithoutPendingReturns = orders.filter(o => {
      ...
      // البيان المفتوح بس: يختفي من الجدول ويفضل في الحاوية الحمرا، بغض النظر
      // عن كونه مُرحّل من بيان أقدم أو لأ — الجدول المفتوح يعرض بس قيد الانتظار
      // ومؤجل، والمرتجع/الجزئي غير المستلم يتابعوه من الحاوية الحمرا لحد ما
      // يوصل المخزن ويتأكد استلامه. البيان المقفول لا يتأثر بالاستبعاد ده إطلاقًا.
