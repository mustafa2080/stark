    ...
    <Card className="border-primary/30 bg-primary/5 p-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-4">
        {(() => {
          ...
          const partialOrdersS = ordersForSettlement.filter(o => o.deliveryStatus === "partial_received" || o.deliveryStatus === "partial_delivered");
          const returnedOrdersS  = ordersForSettlement.filter(o => o.deliveryStatus === "returned");
          const deliveredCODs    = deliveredOrdersS.reduce((sum, o) => {
            const dvr = (o as any).deliveredValueReceived;
            return sum + (dvr != null ? Number(dvr) : Number((o as any).totalPrice ?? 0));
          }, 0);
          const partialCODs = partialOrdersS.reduce((sum, o) => {
          ...
          const collectedCODs = deliveredCODs + partialCODs;
          const totalShipmentValue = collectedCODs;
          return (
            <>
              ...
              <div className="bg-card rounded-md p-3 border border-emerald-900/40">
                ...
                <p className="text-base font-black text-emerald-600 dark:text-emerald-400">{formatCurrency(collectedCODs)}</p>
                <p className="text-[10px] text-emerald-700 dark:text-emerald-600">{deliveredOrdersS.length + partialOrdersS.length} طلبية</p>
              </div>
              ...
              <div className="bg-card rounded-md p-3 border border-border">
                ...
                {effectiveShippingCost === 0 ? (
                  <p className="text-[10px] text-muted-foreground/60">لا توجد رسوم شحن مسجّلة</p>
                ) : (
              ...
              <div className="bg-card rounded-md p-3 border border-primary/40">
                ...
                <p className="text-[10px] text-muted-foreground">مُسلَّم + جزئي − شحن</p>
              </div>
            </>
          );
        })()}
...
export default function ShippingManifestPage() {
  ...
  const filteredManifestOrders = useMemo(() => {
    ...
    // البيان مفتوح (شحنة أصلية دخلت البيان ده من الأول، مش مُرحّلة): المرتجع/
    //   الجزئي (لسه عند المندوب) يظهر في الجدول وفي الحاوية الحمرا مع بعض
    //   (عرض مزدوج — خرج من عند المندوب لسه مش مؤكد). المستخدم محتاج يشوفه في
    //   الجدول قبل القفل عشان يتابع حالته وسط باقي شحنات البيان.
    // البيان بمجرد ما يتقفل: نفس البند يختفي من الجدول تمامًا ويفضل في الحاوية
    //   الحمرا بس — البيان المقفول يقتصر على قيد الانتظار/مؤجل.
    // ⚠️⚠️ البند المُرحّل من بيان قديم مقفول (rolledOver === true): يختفي من
    //   الجدول من ساعة ما البيان الجديد يتفتح — مش لما يتقفل — لأن حدثه المالي
    //   الأصلي اتحسب فعلاً في البيان القديم وقت قفله، فالجدول الجديد المفروض
    //   يعرض بس قيد الانتظار/مؤجل من أول لحظة، والمرتجع المُرحّل يتابعوه من
    //   الحاوية الحمرا بس. (تصحيح 2026-08-27: كان بيتطبق عليه شرط المفتوح
    //   العادي غلط فيظهر فوق زي شحنة أصلية.)
    ...
    const ordersWithoutPendingReturns = orders.filter(o => {
      ...
      // مُرحّل من بيان قديم مقفول → يختفي من الجدول فورًا (بغض النظر عن حالة
      // البيان الحالي مفتوح أو مقفول)، لأن حدثه المالي اتحسب في البيان القديم.
      // مش مُرحّل (أصلي في البيان الحالي) → يظهر في الجدول طول ما البيان مفتوح،
      // ويختفي بس بعد القفل — البيان المقفول التاريخي يقتصر على قيد الانتظار/مؤجل.
      ...
      if (isReturnedOrPartialByDelivery && !isConfirmed) {
        if ((o as any).rolledOver) return false;
    ...
    <div className="manifest-screen print:hidden max-w-5xl mx-auto space-y-5 animate-in fade-in duration-500" dir="rtl">
                ...
                {displayGroups.length === 0 && colFilterHasActive ? (
                  ...
                  <InvoiceGroupDeliveryRow
                    ...
                    courierShippingCost={(() => {
                      ...
                      return manifest.manualShippingCost;
                    })()}
                  />
                  ))}
                  </div>
      ...
      {canViewFinancials && (() => {
        ...
        const ordersForPnl = (manifest.orders ?? []).filter(o => {
          ...
          // القديم). لو مش مُرحَّل (أصلي في البيان الحالي) → يتحسب عادي إلا لو
          // بسبب غير مالي (مفيش حدث مالي حصل أصلاً).
          if (o.deliveryStatus === "returned" && (o as any).returnReceived !== 1) {
            if ((o as any).rolledOver) return false;
            return RETURN_REASONS_FINANCIAL.includes(String((o as any).returnReason ?? ""));
          }
          // ─── جزئي لسه معلَّق عند شركة الشحن (returnReceived !== 1) ───────────
          // نفس منطق المرتجع بالظبط: لو مُرحَّل من بيان قديم مقفول → يتصفّر هنا
          // (القيمة المستلمة الجزئية والشحن اتحسبوا أصلاً في البيان القديم).
          // لو مش مُرحَّل (جزئي أصلي حصل داخل البيان الحالي) → يتحسب عادي زي
          // قبل الإصلاح ده بالظبط (القيمة الجزئية المُسلَّمة فعلية وحقيقية).
          if (
            (o.deliveryStatus === "partial_received" || o.deliveryStatus === "partial_delivered") &&
            (o as any).returnReceived !== 1
          ) {
            return !((o as any).rolledOver);
          }
          return true;
        });
        ...
        const isShippingZeroedRowLocal = (order: any) => {
          ...
          if (st === "postponed" || st === "delayed" || st === "pending") return true;
          if (st === "returned") {
            // سعر الشحن مستحق دايمًا للأسباب المالية التلاتة، بغض النظر عن وجود
            // تحصيل فعلي أو لا. الباك اند (rolledOverShipmentIds) بيمنع الـ fallback
            // القديم من بيان المندوب للبنود المُرحّلة، فمفيش خطر تكرار بعد rollover.
