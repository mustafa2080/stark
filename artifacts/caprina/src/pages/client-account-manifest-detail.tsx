...
export default function ShippingManifestPage() {
  ...
  const filteredManifestOrders = useMemo(() => {
    ...
    //   الحمرا مع بعض (عرض مزدوج — خرج من عند المندوب لسه مش مؤكد). المستخدم
    //   محتاج يشوفه في الجدول قبل القفل عشان يتابع حالته وسط باقي شحنات البيان.
    // البيان بمجرد ما يتقفل: المرتجع/الجزئي اللي لسه عند المندوب (غير مؤكد
    //   الاستلام) يختفي من الجدول تمامًا ويفضل في الحاوية الحمرا بس — الجدول
    //   المقفول يقتصر على قيد الانتظار/مؤجل. (تصحيح 2026-08-27: رجّعنا الاتجاه
    //   الصح بعد لخبطة — الاستبعاد يتطبق عند القفل، مش قبله.)
    ...
    const ordersWithoutPendingReturns = orders.filter(o => {
      ...
      // بعد قفل البيان بس: يختفي من الجدول ويفضل في الحاوية الحمرا، بغض النظر
      ...
      // يوصل المخزن ويتأكد استلامه. البيان المفتوح يعرضه في الجدول عادي.
