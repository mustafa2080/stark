...
export default function ShippingManifestPage() {
  ...
  const filteredManifestOrders = useMemo(() => {
    ...
    const ordersWithoutPendingReturns = orders.filter(o => {
      ...
      // بيتشال من الجدول (ويفضل في الحاوية الحمرا بس) في حالة واحدة بس:
      ...
      //     فرّق بين المرتجع الأصلي (يفضل فوق) والمرتجع المُرحّل.
      // ملحوظة مهمة: isClosed (حالة البيان الحالي) ملهاش دخل في القرار ده —
      // المرتجع الأصلي غير المستلم لازم يفضل ظاهر في جدول "الشحنات في البيان"
      // بتاع نفس البيان حتى بعد ما البيان يتقفل (البيان المقفول أرشيف تاريخي
      // ثابت). اللي بيختفي هو نسخته الجديدة في البيان الجديد (isRolledOver=true
      // على البند الجديد)، مش البند الأصلي في البيان القديم. استخدام isClosed
      // هنا كان بيخفي المرتجع من البيان المقفول نفسه غلط بدل ما يفضل فيه.
      ...
      if (isReturnedOrPartialByDelivery && !isConfirmed && isRolledOver) return false;
