          ...
          <AlertDialogContent>
            ...
            {needsPartialInput && (
              <div className="space-y-1.5 pt-1">
                <label htmlFor={`partial-qty-${order.id}`} className="text-xs font-semibold text-muted-foreground">
                  الكمية/المبلغ المستلم جزئيًا (لا يوجد قيمة مسجّلة لهذا الطلب)
                </label>
                <input
                  id={`partial-qty-${order.id}`}
                  type="number"
                  min={1}
                  inputMode="numeric"
                  value={partialInput}
    ...
    <div className="manifest-screen print:hidden max-w-5xl mx-auto space-y-5 animate-in fade-in duration-500" dir="rtl">
      ...
      <div className="rounded-2xl border border-border bg-gradient-to-br from-violet-500/10 via-violet-500/[0.03] to-transparent overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <span className="text-xl font-black text-violet-300">{filteredManifestOrders.length}</span>
            <ChevronDown
