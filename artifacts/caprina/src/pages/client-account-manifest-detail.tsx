...
const SHIPMENT_STATUS_STYLE: Record<string, { label: string; color: string; bg: string }> = {
  ...
  received:         { label: "مسلَّم ✓",           color: "text-emerald-800 dark:text-emerald-300", bg: "border-emerald-400 dark:border-emerald-700 bg-emerald-100 dark:bg-emerald-900/40" },
  ...
  delivered:        { label: "مسلَّم ✓",           color: "text-emerald-800 dark:text-emerald-300", bg: "border-emerald-400 dark:border-emerald-700 bg-emerald-100 dark:bg-emerald-900/40" },
...
export default function ShippingManifestPage() {
  ...
  const filteredManifestOrders = useMemo(() => {
    ...
    const ordersWithoutPendingReturns = orders.filter(o => {
      ...
      //
      // ⚠️⚠️ ملاحظة (2026-09-03، تحقيق شحنة "أحمد" CAM-84-003): جُرّب تعديل هنا
      // بافتراض إن deliveryStatus="delivered" على بند rolledOver يبقى معناه
      // حدث تسليم جديد حصل في البيان الحالي — الافتراض ده غلط. التحقق من
      // added_at الفعلي في قاعدة البيانات أثبت إن deliveryStatus="delivered"
      // كان موجود بالفعل من نفس لحظة إنشاء أول بند (في أقدم بيان)، يعني القيمة
      // اتحسبت هناك بالفعل ولم يحدث أي حدث مالي جديد في البيان الحالي — التكرار
      // كان ناتج عن باگ قديم في منطق الترحيل (اتصلح في rolloverPendingItemsToNewManifest
      // الحالية، لكن الصفوف القديمة المتولدة وقت الباگ لسه موجودة في الداتا).
      // فالشرط الأصلي (استبعاد أي حالة نهائية غير delayed/postponed/pending
      // لأي بند rolledOver) هو الصح فعليًا، ورجعناه زي ما كان.
