...
function SocialFloat({ darkMode }: { darkMode: boolean }) {
  ...
  return (
    <div className="fixed left-5 bottom-8 z-50 flex flex-col items-center gap-3">
      {/* Social icons */}
      <div className={`flex flex-col items-center gap-2 transition-all duration-500 ${open ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 translate-y-4 pointer-events-none"}`}>
        {socials.map((s) => (
          <a
            key={s.label}
            href={s.href}
            ...
            title={s.label}
            className="w-11 h-11 rounded-full flex items-center justify-center text-white shadow-lg transition-all duration-200 hover:scale-110 active:scale-95"
            style={{
              background: s.color,
      ...
      <button
        onClick={() => setOpen(v => !v)}
        className="w-13 h-13 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 hover:scale-110 active:scale-95"
        style={{
