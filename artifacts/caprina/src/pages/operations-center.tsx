...
// ── أيقونة واتساب الحقيقية (نفس الموجودة في layout.tsx) ───────────────────────
          ...
          <CardHeader className="pb-2 space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-blue-500/10 px-2.5 py-1 text-[10px] font-black text-blue-600 dark:text-blue-400">إجمالي الإيرادات: {revenueTrendPnlLoading ? "—" : fc(revenueTrendPnlSummary?.totalRevenue ?? 0)}</span>
                {revenueTrendFilter.type === "custom" && (
                  <Button
                    size="sm"
                    variant="ghost"
                  ...
                  {m.shippingCompanyPhone && (
                    <div className="flex items-stretch gap-2">
                      <a
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-amber-500/30 bg-background/60 py-1.5 text-xs font-bold text-amber-700 dark:text-amber-300 transition-colors hover:bg-amber-500/15"
                      >
                      ...
                      <a
                        href={buildWhatsAppLink(
                          m.shippingCompanyPhone,
                          `أهلاً ${m.shippingCompanyName} 👋\nبخصوص البيان رقم ${m.manifestNumber} — مفتوح من ${m.daysOpen >= 1 ? `${m.daysOpen} يوم` : `${m.hoursOpen} ساعة`}، برجاء المتابعة وتسليم الحساب.`,
                        )}
                        target="_blank"
                        rel="noopener noreferrer"
                        title="مراسلة عبر واتساب"
                        aria-label={`مراسلة ${m.shippingCompanyName} عبر واتساب`}
                        className="shrink-0 inline-flex items-center justify-center rounded-lg border border-emerald-500/40 bg-emerald-500/15 px-3 text-emerald-600 dark:text-emerald-400 transition-colors hover:bg-emerald-500/30"
                      >
              ...
              <table className="w-full text-xs min-w-[420px]">
                <thead className="sticky top-0 z-10 bg-card">
                  <tr className="text-muted-foreground border-b">
                    ...
                    <th className="text-right font-medium pb-2">إيراد العميل</th>
